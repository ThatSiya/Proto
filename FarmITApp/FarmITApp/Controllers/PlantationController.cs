...
namespace FarmITApp.Controllers
{
    public class PlantationController : Controller
    {
        ...
        public ActionResult Index(string search, int? page, string croptype)
        {
            ...
            //select plantations
            var plantations = db.Plantations.Include(p => p.CropCycle).Include(p => p.CropType).Include(p => p.Field).Include(p => p.FieldStage).Include(p => p.Unit).Include(p => p.Unit1);
        ...
        public ActionResult Create([Bind(Include = "PlantationID,FieldID,CropTypeID,CropCycleID,FieldStageID,DatePlanted,RefugeSeedAmntUsed,RefugeUnit,RefugeAreaHectares,ExpectedYieldQnty,YieldUnit,DateHarvested,PlantationStatus")] Plantation plantation)
        {
